...
```{r lac.selec, include=FALSE, tidy=TRUE}
lac <- gene.names %>% filter(str_detect(name, "lac"))
#knitr::kable(lac)
```

```{r interactions, include=FALSE, tidy=TRUE}
...
#dim(lac.deep.inter.num)
```
```{r naming, fig.margin=TRUE, include=FALSE}
# Prepare the format
...
```{r the.graph, echo=FALSE, fig.align="c", fig.fullwidth=TRUE, fig.height=10, message=FALSE, warning=FALSE}
library(igraph)
...
```{r activated, echo=FALSE, fig.align="c", fig.fullwidth=TRUE, fig.height=10, message=FALSE, warning=FALSE}
